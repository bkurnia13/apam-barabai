<div class="main-content">
    <div class="container-fluid">
        <?= 'Selamat Datang ' . $user['name'] . ' sebagai ' . $dashboard; ?>
    </div>
</div>

<footer class="footer">
    <div class="w-100 clearfix">
        <span class="text-center text-sm-left d-md-inline-block">Copyright © 2020 | Bayu Kurnia Dwi Putra</span>
        <span class="float-none float-sm-right mt-1 mt-sm-0 text-center">Template by <a href="http://lavalite.org/" class="text-dark" target="_blank">ThemeKit</a></span>
    </div>
</footer>

<script src="https://code.jquery.com/jquery-3.3.1.min.js"></script>
<script>
    window.jQuery || document.write('<script src="<?= base_url('assets/'); ?>/src/js/vendor/jquery-3.3.1.min.js"><\/script>')
</script>